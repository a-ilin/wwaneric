...
class Pdu_Address {
    ...
    std::string  value_;   // Address-Value
...
public:
    ...
    enum {
        // type-of-number
        TN_UNKNOWN = 0, TN_INTERNATIONAL = 1, TN_NATIONAL = 2, TN_NETWORK = 3, TN_SUBSCRIBER = 4, 
        TN_ALPHANUMERIC = 5, TN_ABBREVIATED = 6, TN_RESERVED = 7,
        // numbering-plan-identification
        NP_UNKNOWN = 0, NP_TELEPHONE = 1, NP_DATA = 3, NP_TELEX = 4, NP_NATIONAL = 8, NP_PRIVATE = 9,
        NP_ERMES = 10, NP_RESERVED = 15
    };
    ...
    const std::string &getValue (void ) const { return value_; }
    const char *getValueAsCStr (void ) const { return value_.c_str(); }
